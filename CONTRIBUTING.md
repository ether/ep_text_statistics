# Contributing to ep_text_statistics

Thanks for helping improve `ep_text_statistics` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_text_statistics` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_text_statistics from your local clone
pnpm run plugins i --path ../ep_text_statistics

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_text_statistics/` are picked up after a server restart.
...
* Etherpad version, ep_text_statistics version, browser + OS
* Steps to reproduce
